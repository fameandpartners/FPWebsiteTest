...
def getLiEle(driver, xpath):
    elements = driver.find_element_by_xpath(
        xpath).find_elements_by_tag_name('li')
    element = driver.find_element_by_xpath(
        xpath + '/li[' + str(random.randint(1, len(elements))) + ']')
    return element
...
def main(argv):
    ...
    FILTER_TEXT = [
        ...
        'FIT']
    COLORS_KIND_DICT = {
        'White / Ivory': "245, 244, 243",
        'Tan / Sand': "230, 212, 192",
        'Brown': "181, 101, 29",
        'Blush': "222, 93, 131",
        'Pink': "244, 88, 166",
        'Orange': "255, 165, 0",
    ...
    dropdown_list(
        driver,
        '//*[@id="root"]/main/div/section[1]/div/div[1]/div/div[1]',
        log,
        0,
        2)
    sleep(3, log)
...
# -*-*-*-*
    ...
    for i in range(2, FILTER_KIND + 1):
        ...
        if i <= filter_len + 1:
            ...
            ele = getLiEle(
                driver,
                '//*[@id="root"]/main/div/section[1]/div/div[2]/div[' +
                str(i) +
                ']/section[2]/ul')
            ...
            break
        sleep(2, log)
    get_return_code(log, '筛选结果:' + filter_res_text)
    sleep(2, log)
...
# -*-*-*-*
    ...
    for i in range(2, clicked_filters_len + 2):
        clicked_filter_all_xp = '//*[@id="root"]/main/div/section[1]/div/div[2]/div[' + str(
            i) + ']/section[1]/span'
        if check_texts[i - 1] in COLORS_KIND_DICT:
            color_ele = find_element(
                driver, clicked_filter_all_xp, 'xpath', log)
            sleep(2, log)
            ...
            if COLORS_KIND_DICT[check_texts[i - 1]] in color_text:
                ...
                get_return_code(log, '已选筛选选项[%s]收起显示错误' %
                                check_texts[i - 1], _code=500)
        else:
            assert_text(driver,
                        check_texts[i - 1],
                        'xpath',
                        clicked_filter_all_xp,
                        log,
                        '已选筛选选项[%s]收起显示正确' % check_texts[i - 1],
                        '已选筛选选项[%s]收起显示错误' % check_texts[i - 1])
