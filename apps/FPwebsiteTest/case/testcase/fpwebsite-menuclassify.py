...
def main(argv):
    ...
    shop_elements = {'Evening': base_url + 'dresses/evening',
    ...
    weddings_elements = {'Bridesmaid Dresses': base_url + 'dresses/bridesmaid',
    ...
    about_elements = {'Why custom': base_url + 'about', '$20 off to try custom': base_url + 'account/signup',
                      'Contact Us': base_url + 'contact'}

    shop_all = {"//div[@class='jsx-81871819 HeaderMegaMenuContainer']//div[1]//ul[1]//li[1]//a[1]": base_url + 'dresses',
    ...
    for kind in menu_kinds:
        if kind == 'Shop':
            for link in shop_elements:
                ...
                # 检测菜单栏名称是否显示正确
                assert_text(driver, link, 'link_text', link, log, '菜单栏名称[%s]显示正确' % link, '菜单栏名称[%s]显示错误' % link)
                # 检测对应的url是否正确
                # try:
            ...
            for link in shop_all:
                xp = driver.find_element_by_link_text(kind)
                ActionChains(driver).move_to_element(xp).perform()
                assert_text(driver, 'Shop All', 'xpath', link, log, '菜单栏名称Shop All显示正确', '菜单栏名称Shop All显示错误')
                # try:
