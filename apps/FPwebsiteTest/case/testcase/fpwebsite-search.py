...
def main(argv):
    FILTER_KIND = ['COLLECTION', 'DELIVERED IN', 'PRICE', 'COLORS', 'OCCASION', 'SILHOUETTE',
                   'BODYSHAPE', 'FIT', 'STYLE', 'PATTERN', 'OTHER', 'EVENT', 'NECKLINE',
                   'SLEEVE', 'LENGTH', 'FABRIC', 'DESIGN FEATURES']
    ...
    # 搜索关键词 red
    search_btn = find_element(driver, '//*[@id="3xmW4MhfBeGEUoUYIYEQEI"]/header/nav/ul[2]/li[2]/a', 'xpath', log)
    ...
    assert_text(driver, base_res + '"red"', 'xpath', '//*[@id="root"]/main/section/h1/span', log, "The search result's "
                                                                                                  "context is right!",
                "The search result's context is wrong!")
    # # 检测搜索关键词 red 右侧筛选项是否正确
    assert_text(driver, 'FILTER BY:', 'xpath', '//*[@id="root"]/main/div/section[1]/div/div[2]/div[1]/p', log,
                'FILTER BY显示正确', 'FILTER BY显示错误')
    ...
    if len(elements) == len(FILTER_KIND):
        ...
        get_return_code(log, '筛选项长度不一致，筛选项长度为[%d],实际筛选项长度为[%d]' % len(FILTER_KIND) % len(elements), _code=500)
    sleep(5, log)
    ...
    driver.execute_script('arguments[0].click()', search_btn)
    text_xp = driver.find_element_by_xpath('/html[1]/body[1]/div[3]/header[1]/nav[1]/ul[2]/div[1]')
    ActionChains(driver).move_to_element(text_xp).send_keys('dfagsgsagtet').perform()
    ActionChains(driver).move_to_element(text_xp).key_down(Keys.ENTER).perform()
    result_xp_text = find_element(driver, '//*[@id="root"]/main/section/h1/span', 'xpath', log).text
    res_wrong_xp = find_element(driver, '//*[@id="root"]/main/div/section[2]/div/h4/span/a', 'xpath', log)
    if result_xp_text == base_res + '"dfagsgsagtet"' and res_wrong_xp.text == 'View our best selling dresses.':
        get_return_code(log, "The search result's context is right!")
    else:
        get_return_code(log, "The search result's context is wrong!", _code=500)
    ...
    ActionChains(driver).move_to_element(text_xp).key_down(Keys.ENTER).perform()
    result_xp = find_element(driver, '//*[@id="root"]/main/section/h1/span', 'xpath', log)
